...
| **M16** | [Evidence routine as background producer](11-background-routine.md) · [design](design/11-background-routine.md) | Gathering stops blocking testing | **Shipped behind `--gather-background`**: producer thread, allowlist handover, dedupe under two writers, and a producer-side budget reserve. **Paired campaign run 2026-08-20**: the consumer never stalled and evidence went 158h stale → 0.02h (+18 artifacts) against +0 on the baseline. Criterion 3 half met — the hypothesis pool did not refill, because the sweep outlived the campaign. Criterion 1's premise did not reproduce: the baseline declined to gather at every step rather than blocking on it |
