...
| **M16** | [Evidence routine as background producer](11-background-routine.md) · [design](design/11-background-routine.md) | Gathering stops blocking testing | **Shipped behind `--gather-background`**: producer thread, allowlist handover, dedupe under two writers, and a producer-side budget reserve. **Campaign runs 2026-08-20**: all exit criteria met. A 51-minute run produced two complete sweeps, 18 hypotheses minted and 18 consumer steps with zero `analyze_competition` dispatched by the campaign; evidence 158h stale → 0.01h against +0 on the baseline. Two follow-ups open: the producer's output does not move `viable_hypothesis_count`, so it re-sweeps indefinitely; and criterion 1's premise did not reproduce — the baseline declined to gather at every step rather than blocking on it |
