...
| **M22** | [An inference without evidence is a guess](17-dataset-understanding.md) | **Ground truth.** Every result above is a conclusion about whichever column the profiler happened to pick | Not started. PR #139 fixed the inference itself — rogii inferred `EGFDU`, a horizon depth, because column roles were unioned across per-entity tables, and it stayed invisible because `profile.json` was written 2026-08-02 and never re-derived. That removes two ways of guessing wrong; it does not add a check that the answer is right, which is what this milestone is |
...
| **M16** | [Evidence routine as background producer](11-background-routine.md) · [design](design/11-background-routine.md) | Gathering stops blocking testing | **Shipped behind `--gather-background`**: producer thread, allowlist handover, dedupe under two writers, and a producer-side budget reserve. **Campaign runs 2026-08-20**: all exit criteria met. A 51-minute run produced two complete sweeps, 18 hypotheses minted and 18 consumer steps with zero `analyze_competition` dispatched by the campaign; evidence 158h stale → 0.01h against +0 on the baseline. One follow-up open, and one fixed: the producer's output did not move `viable_hypothesis_count`, so it re-swept indefinitely — it now latches that clause when a sweep fails to move it — and the latch suppresses only that clause, because the gate returns at the first one that fires and a latched `thin` was silencing day-old evidence with it; whether viability is the right gate at a 153-row pool is left to a milestone that can weigh M21's ratchet; and criterion 1's premise did not reproduce — the baseline declined to gather at every step rather than blocking on it |
| **M17** | [Run until plateau or goal](12-run-until-done.md) · [design](design/12-run-until-done.md) | Campaigns end on the objective, not a step counter | **Shipped 2026-08-20; criteria 3–5 demonstrated** over eleven campaigns, and **`max_steps` ended none of them** — the counter that ended every campaign in these evidence logs no longer does. Criteria 1–2 undemonstrated, with three blockers found and fixed on the way: a `plateau` that could never fire on a metric of real magnitude; a score recorded as `cv_score` that no `rmse` target could match; and a `run_experiment` path invisible to the series because the id it had already stamped on its own refs was never surfaced. What remains is a codegen defect — no `train.py` on that path, so every execution fails before it scores ([design §13](design/12-run-until-done.md)) |
