...
| **M17** | [Run until plateau or goal](12-run-until-done.md) · [design](design/12-run-until-done.md) | Campaigns end on the objective, not a step counter | **Shipped 2026-08-20; criteria 3–5 demonstrated** over ten campaigns, and **`max_steps` ended none of them** — the counter that ended every campaign in these evidence logs no longer does. Criteria 1–2 undemonstrated: both blockers found and fixed (a `plateau` that could never fire on a real magnitude; a score recorded as `cv_score` that no `rmse` target could match), but a campaign must choose `run_plan` to exercise them and the policy kept choosing `run_experiment`, which writes no execution outcome and so can never feed the series at all ([design §13](design/12-run-until-done.md)) |
