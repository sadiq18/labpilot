...
| **M17** | [Run until plateau or goal](12-run-until-done.md) · [design](design/12-run-until-done.md) | Campaigns end on the objective, not a step counter | **Shipped 2026-08-20; criteria 3–5 demonstrated** over seven campaigns, and **`max_steps` ended none of them** — the counter that ended every campaign in these evidence logs no longer does. Criteria 1–2 open: a real score of 1.65 against a 2.0 target could not fire `metric_target`, because the run recorded it as `cv_score` and no `rmse` target can match that. Found on the way: `plateau` could never fire on a metric of real magnitude, and the codegen gate enforced a PEP 723 rule no prompt stated — every experiment rejected, across two unrelated models ([design §13](design/12-run-until-done.md)) |
