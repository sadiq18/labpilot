...
| **M17** | [Run until plateau or goal](12-run-until-done.md) · [design](design/12-run-until-done.md) | Campaigns end on the objective, not a step counter | **Implementation shipped 2026-08-20**: `--max-steps` is unbounded by default, `needs_guidance` pauses a campaign that cannot progress, and a goal-progress line prints each step. Found on the way: `plateau` could not fire on a metric of any real magnitude — an absolute `1e-6` against a raw spread — so the stop the unbounded loop depends on had never worked. **Exit criteria 1–3 not demonstrated**, all three needing a campaign log |
