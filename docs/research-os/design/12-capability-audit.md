...
| `implement` | **partial** — confirmed, not "real": the M19-fixed codegen path exists but `ImplementationSpecialist`'s `prefer_patch` shortcut skips it by default whenever the workspace already has code, silently no-opping on `train.py` while still reporting success (autonomy-roadmap §"implement: a second hollow path") | **`description`, not `technique`** — corrected once the fixture was built. The `technique` kwarg never reaches the codegen prompt on this path (`build_v1_task_context` writes agent metadata onto the `ResearchTask`; `CodeEngineeringCapability._write` reads `plan.metadata`), so the prompt always renders `Technique: —` and only `description` — via `goal` — conditions the output. An earlier draft of this row said `technique`; following it produces a contract test that correctly fails. | Extended `FakeCodegenLLM` (§6.2, which reaches `description` through its `Goal:` fallback) **plus `force_rewrite=True` in the fixture inputs** | Two ways to be vacuous here, both hit in practice: without `force_rewrite=True` the test passes against the `prefer_patch` no-op on any workspace that already has `pipeline/train.py`; and with `varies_by=["technique"]` it would assert on an input the tool cannot see |
